...
int main()
{
    Graph *G = create_map(); //generate_map
    ...
    SPG Result = dijkstra(G,source);                    // Passing graph and source node as parameters and returns a structure (SPG)
                                                        // which consists of distance from source and parent data for each vertex

    display_SPG(Result,destination_id);                 // Function which prints the shortest path graph, in basic form (i.e. A-->B-->E-->T)
